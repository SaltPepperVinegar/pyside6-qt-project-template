...
You can treat this repo as an “empty shell” app with the architecture already in place, ready to be filled with your own features.
...
## 🛠️ PyCharm File Watcher for Automatic UI Generation
...
2. Click **+** (top right) → choose **Custom**
3. Fill in:
   - **Name**: `Build UI`
...
## 🧱 Directory Structure & Recommended Usage
...
- **`main_window.py`**  
  ...
  - **Recommended usage**:
    ...
    - Keep only “shell-level” responsibilities here: menu bar, toolbars, main layout, global shortcuts, etc.
...
- Do **not** hand-edit or store `.ui` files inside `ui_generated` — that directory is for generated code only.
...
- Contains “larger” chunks of UI with full features, usually:
  - inheriting from a class in `ui_generated.panels.*`, or
  - building the layout manually while still acting as a logical “page” or “panel”.
...
- Consider using Qt’s resource system (`.qrc`) or maintain a simple path helper to keep resource paths consistent.
...
- **`tools/build_ui.py`**  
  ...
  - Example mapping:
    - `ui/panels/project_panel.ui` → `ui_generated/panels/project_panel_ui.py`
...
5. **Maintain structural consistency**  
   - When adding new features, decide upfront whether something is a panel, widget, service, or core utility, instead of putting everything into `main_window.py` or a single “god file”.
